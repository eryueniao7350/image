  ...
  return `<header id="site-header" class="bp-header">
    <div class="bp-header-inner">
      ...
      <nav class="bp-nav-links">
        ...
        <a href="${BASE}/best/" class="bp-nav-link bp-nav-link--active">Best Tools</a>
  ...
  const groupsHtml = groups.map((g) => {
    const items = g.slugs
      .map((slug) => {
        ...
        return `<a href="${BASE}/best/${esc(slug)}/" class="bp-card" style="display:flex;align-items:center;justify-content:space-between;padding:10px 16px;text-decoration:none">
          <div>
...
function buildScenarioHtml(scenario, skills, assetTags, allScenarios) {
    ...
    return `<div class="bp-card" style="margin:16px 0">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px">
          <div>
            ...
            <a class="bp-card-title" href="${BASE}/skill/${esc(s.repo_full_name)}/" style="font-size:18px;display:inline">${esc(s.repo_name)}</a>${isNew ? `<span class="bp-badge-new">NEW</span>` : ""}
        ...
        <div style="margin-top:10px;display:flex;gap:12px">
          <a href="${BASE}/skill/${esc(s.repo_full_name)}/" style="color:var(--bp-link);font-size:13px;text-decoration:none">View Details &rarr;</a>
  ...
  const compRows = skills.map((s) => {
    return `<tr>
          <td><a href="${BASE}/skill/${esc(s.repo_full_name)}/" style="font-weight:500">${esc(s.repo_name)}</a></td>
  ...
  const relatedHtml = (scenario.related || [])
    .map((slug) => {
      ...
      if (!rel) return null;
      return `<a href="${BASE}/best/${esc(slug)}/" class="bp-related-tag">${esc(rel.title)}</a>`;
    })
    .filter(Boolean)
    .join("\n        ");
...
<body class="bp-body">
  ...
  <div class="bp-container">
      ...
      <nav class="bp-breadcrumb">
        <a href="${BASE}/" data-zh="首页" data-en="Home">Home</a>
        <span style="margin:0 6px">&gt;</span>
        <a href="${BASE}/best/" data-zh="最佳工具" data-en="Best Tools">Best Tools</a>
        <span style="margin:0 6px">&gt;</span>
